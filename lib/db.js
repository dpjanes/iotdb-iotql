        ...
        if (v.all) {
            if ((v.band === "istate") || (v.band === "ostate")) {
                ...
                attributes.map(function (attribute) {
                    ...
                    if (purpose && purpose.match(/^iot-attribute:/)) {
                        purpose = purpose.replace(/^iot-attribute:/, '');
                    }

                    /* TO BE REVISITED
                    var roles = _.ld.list(attribute, 'iot:role', []);
                    console.log("ROLES", roles);

                    if (v.band === "istate") {
                        var is_reading = (roles.length === 0) || (roles.indexOf('iot-attribute:role-reading') > -1);
                        if (!is_reading) {
                            return;
                        }
                    }
                    ...
                    if (v.band === "ostate") {
                        var is_control = (roles.length === 0) || (roles.indexOf('iot-attribute:role-control') > -1);
                        if (!is_control) {
                            return;
                        }
                    }
