...
DB.prototype.run_statement_create_trigger = function (trigger_statement, callback) {
    ...
    if (trigger_where) {
        self._monitor_trigger(store, transporter);
        callback(null, null);
    } else {
 ...
 */ 
DB.prototype._monitor_trigger = function(store, transporter) {
    var self = this;
...
DB.prototype._monitor_updated = function(store, transporter, ud) {
    ...
    self.fetch_bands(transporter, ud.id, [ "istate", "ostate", "meta", "model" ], function (error, rowd) {
        if (error) {
            console.log("_monitor_trigger: cannot fetch bands: %s", _.error.message(error));
            return;
        }
        ...
        _.mapObject(self.scened, function(scene, scene_name) {
            ...
            if (!scene_statements) {
                console.log("_monitor_trigger: no begin-end?: %s", _.error.message(error));
                return;
            }
