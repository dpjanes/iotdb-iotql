...
/**
 *  DO statement
 *  - if the band is anything but 'scene', we are calling 
 *    a operation just like SELECT
 *
 *  The difference between SELECT and DO code is horrible
 *  and probably should be rationalized in the future.
 */
DB.prototype.run_statement_do = function (do_statement, callback) {
    ...
    var scene_module = do_statement['module'].toLowerCase();
    if (scene_module !== "scene") {
        var v = {
            compute: {
                operation: scene_name,
                module: scene_module,
                operands: do_statement.operands,
            },
        };
